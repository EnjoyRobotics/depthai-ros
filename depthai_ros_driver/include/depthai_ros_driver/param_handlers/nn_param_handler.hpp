...
namespace nn
{
enum class NNFamily { Segmentation, Mobilenet, Yolo, CreStereo };
}
...
public:
  ...
  void setNNParams(
    nlohmann::json data,
    std::shared_ptr<dai::node::MobileNetSpatialDetectionNetwork> nn);
  void setNNParams(nlohmann::json data, std::shared_ptr<dai::node::YoloSpatialDetectionNetwork> nn);
  void setNNParams(nlohmann::json data, std::shared_ptr<dai::node::CreStereoDepthNetwork> nn);
  ...
  void setYoloParams(nlohmann::json data, std::shared_ptr<T> nn)
  {
    ...
    std::map<std::string,
      std::vector<int>> anchor_masks = {{"side13", {3, 4, 5}}, {"side26", {1, 2, 3}}};
  ...
  template<typename T>
  void parseConfigFile(const std::string & path, std::shared_ptr<T> nn,
    std::shared_ptr<dai::node::ImageManip> imageManip)
  {
    ...
    if (data.contains("model") && data.contains("nn_config")) {
      ...
      nn->setNumInferenceThreads(declareAndLogParam<int>("i_num_inference_threads", 1));
...
private:
  void setImageManip(const std::string & model_path,
    std::shared_ptr<dai::node::ImageManip> imageManip);
